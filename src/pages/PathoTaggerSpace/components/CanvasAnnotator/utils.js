...
export const drawPoint = options => {
  const { position, color, label, circleRadius = 2, id, tagInfo } = options
...
export const drawEllipse = options => {
  const { top, left, rx, ry, color, label, strokeWidth = 1, isfinish = true, id, tagInfo } = options
...
export const drawPolygon = options => {
  const { points, label, color, strokeWidth = 1, id, tagInfo } = options
